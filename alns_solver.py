...
class UpdatedALNSSolver:
    ...
    def solve_optimized(
        ...
        try:
            initial_optimized = self.global_optimizer.optimize_if_needed(
                -1, current_solution, -1, distance_matrix, eta_matrix
            )
            
            if initial_optimized.average_cost_per_parcel < current_solution.average_cost_per_parcel * 0.99:
        ...
        for iteration in range(max_iterations):
            try:
                ...
                repaired = self._apply_alns_operator(
                    current_solution, iteration, consecutive_no_improvement,
                ...
                globally_optimized = self.global_optimizer.optimize_if_needed(
                    iteration, repaired, iterations_since_improvement,
                    distance_matrix, eta_matrix
                )
                
                # Evaluate solution
                globally_optimized.calculate_cost_and_time(distance_matrix, eta_matrix)
                globally_optimized.is_feasible = self._check_feasibility(globally_optimized)
                
                # Acceptance criterion based on cost per parcel
                if self._should_accept_solution(globally_optimized, current_solution, best_solution):
                    current_solution = globally_optimized
                    consecutive_no_improvement = 0
                    
                    # Update best solution if better
                    if (globally_optimized.is_feasible and 
                        (not best_solution.is_feasible or 
                         globally_optimized.average_cost_per_parcel < best_solution.average_cost_per_parcel)):
                    
                        improvement = ((best_solution.average_cost_per_parcel - globally_optimized.average_cost_per_parcel) / 
                                     best_solution.average_cost_per_parcel) * 100
                        ...
                        if improvement > 5.0:  # Significant improvement
                            if verbose_logging or iteration % 25 == 0:
                                print(f"Iteration {iteration}: Significant improvement ({improvement:.1f}%)")
                                self._print_solution_stats(best_solution, f"Iteration {iteration}", verbose_logging)
                        elif improvement > 1.0:  # Minor improvement
                            if verbose_logging:
                                print(f"Iteration {iteration}: Minor improvement ({improvement:.1f}%)")
                else:
                    consecutive_no_improvement += 1
                ...
                if (len(best_solution.routes) == 1 and 
                    best_solution.routes[0].get_utilization_percentage() > 80 and
                    iteration - last_improvement > 10):
                    if verbose_logging:
        ...
        if best_solution.is_feasible and len(best_solution.routes) > 1:
            if verbose_logging:
            ...
            try:
                ...
                if (final_optimized.is_feasible and 
                    final_optimized.average_cost_per_parcel < best_solution.average_cost_per_parcel * 0.99):
    ...
    def _apply_alns_operator(self, solution: Solution, iteration: int, 
                           consecutive_no_improvement: int, distance_matrix: Dict, 
                           eta_matrix: Dict) -> Optional[Solution]:
        """Apply ALNS operator based on current state"""
        
        # If we have single route and it's well-utilized, focus on simple operations
        if (len(solution.routes) == 1 and 
            solution.routes[0].get_utilization_percentage() > 70):
            return self._apply_simple_destroy_repair(solution, distance_matrix, eta_matrix)
    ...
    def _apply_simple_destroy_repair(self, solution: Solution, distance_matrix: Dict, 
                                   eta_matrix: Dict) -> Optional[Solution]:
        ...
        try:
            ...
            return self.operators.greedy_insertion(destroyed, distance_matrix, eta_matrix)
        except Exception as e:
            return None

    def _apply_diversification(self, solution: Solution, distance_matrix: Dict, 
                             eta_matrix: Dict) -> Optional[Solution]:
    ...
    def _apply_destroy_repair(self, solution: Solution, distance_matrix: Dict, 
                            eta_matrix: Dict) -> Optional[Solution]:
    ...
    def _apply_route_modification(self, solution: Solution, distance_matrix: Dict, 
                                eta_matrix: Dict) -> Optional[Solution]:
        """Apply route modification operations"""
        try:
            if self.random.random() < 0.7:
                return self.operators.route_splitting_operator(solution, distance_matrix, eta_matrix)
            else:
                return self.operators.parcel_relocation_operator(solution, distance_matrix, eta_matrix)
        except Exception as e:
            return None

    def _apply_local_search(self, solution: Solution, distance_matrix: Dict, 
                          eta_matrix: Dict) -> Optional[Solution]:
        """Apply local search operations"""
        try:
            return self.operators.local_parcel_swap(solution, distance_matrix, eta_matrix)
        except Exception as e:
            return None
    ...
    def _check_feasibility(self, solution: Solution) -> bool:
        ...
        return (
            ...
            solution.validate_knock_constraints(self.config.max_knock) and
            solution.validate_time_window(self.config.time_window_seconds)
        )

    def _should_accept_solution(self, new_solution: Solution, current_solution: Solution, 
        ...
        if new_solution.is_feasible:
            ...
            if new_solution.average_cost_per_parcel < current_solution.average_cost_per_parcel:
                return True
    ...
    def _print_final_stats(self, solution: Solution):
        ...
        if solution.is_feasible:
            ...
            print(f"📈 Average Utilization: {cost_metrics['utilization_stats']['average_utilization']:.1f}%")
            print(f"⚠️  Underutilized Routes: {cost_metrics['utilization_stats']['underutilized_routes']}")
            
            # Time window and knock analysis
            time_violations = [r for r in solution.routes 
    ...
    def format_output(self, solution: Solution, distance_matrix: Dict, eta_matrix: Dict) -> Dict:
        ...
        for pickup_id, vehicle_ids in solution.pickup_assignments.items():
            ...
            for vehicle_id in vehicle_ids:
                ...
                if route:
                    parcels_count = sum(1 for p in route.parcels 
                                      if self._get_parcel_pickup_id(p, solution) == pickup_id)
                    vehicles_info.append({
                        "vehicle_id": vehicle_id, 
                        "parcels_from_pickup": parcels_count
                    })
        ...
        for route in solution.routes:
            ...
            formatted_route = {
                ...
                "total_cost_kt": route.total_cost / 1000,  # Convert to KT
                ...
                "time_window_feasible": route.is_time_feasible(self.config.time_window_seconds),
                "is_optimized": route.is_optimized,
                "parcels_per_pickup": {
                    pickup_id: len([p for p in route.parcels 
                                  if self._get_parcel_pickup_id(p, solution) == pickup_id])
        ...
        time_window_violations = [r for r in solution.routes 
                                if not r.is_time_feasible(self.config.time_window_seconds)]
        ...
        return {
            ...
            "results_summary": {
                ...
                "max_route_duration_seconds": max(
                    (getattr(r, "total_duration", 0) for r in solution.routes), default=0
                ),
            ...
            "analysis": {
                "capacity_utilization": {k.value: v for k, v in capacity_utilization.items()},
                "knock_analysis": knock_analysis,
                "time_analysis": {
                    "total_duration_hours": round(total_duration_seconds / 3600, 2),
                    "max_route_duration_hours": round(
                        max((getattr(r, "total_duration", 0) for r in solution.routes), default=0) / 3600, 2
                    ),
                    "time_window_hours": self.config.time_window_hours,
...
class UpdatedConstructionHeuristic:
    ...
    def greedy_construction(self, pickup_terminals: List[PickupTerminal], 
        ...
        sorted_terminals = sorted(pickup_terminals, key=lambda x: len(x.parcels), reverse=True)
        ...
        for terminal in sorted_terminals:
            ...
            while remaining_parcels:
                ...
                best_route = self._create_best_route(
                    remaining_parcels, terminal, vehicle_counter, distance_matrix, eta_matrix
                )
    ...
    def _pack_parcels_for_cost_efficiency(self, parcels: List[Parcel], vehicle_spec: VehicleSpec,
        ...
        for parcel in sorted_parcels:
            if current_size + parcel.size <= vehicle_spec.capacity:
                ...
                temp_cost_per_parcel = vehicle_spec.calculate_cost_per_parcel(len(temp_parcels))
                
                # Simple time check
                if self._estimated_time_feasible(temp_parcels, terminal, distance_matrix, eta_matrix):
    ...
    def _estimated_time_feasible(self, parcels: List[Parcel], terminal: PickupTerminal,
                               distance_matrix: Dict, eta_matrix: Dict) -> bool:
        ...
        for parcel in parcels:
            ...
            travel_time = eta_matrix.get(current_location, {}).get(delivery_location, 300)
...
class UpdatedALNSOperators:
    ...
    def random_removal(self, solution: Solution, removal_rate: float = 0.3) -> Solution:
        ...
        to_remove = self.random.sample(all_parcels, min(num_to_remove, len(all_parcels)))

        # Remove parcels from routes
        for parcel, route_idx in to_remove:
            route = new_solution.routes[route_idx]
            route.remove_parcels([parcel])
        ...
        for route in new_solution.routes:
            ...
            for parcel in route.parcels:
                ...
                for terminal in new_solution.pickup_terminals:
                    if (parcel in terminal.parcels and 
                        terminal.pickup_id in terminals_to_remove):
    ...
    def greedy_insertion(self, solution: Solution, distance_matrix: Dict, 
        ...
        unassigned = sorted(new_solution.unassigned_parcels, key=lambda x: x.size, reverse=True)
    ...
    def _quick_time_check(self, route: Route, parcel: Parcel, distance_matrix: Dict, 
                         eta_matrix: Dict) -> bool:
        ...
        additional_time = eta_matrix.get(last_location, {}).get(parcel.delivery_location, 300)
        
        estimated_total_time = route.total_duration + additional_time
        return estimated_total_time <= self.config.time_window_seconds

    def _create_new_route_for_cost(self, parcel: Parcel, solution: Solution,
                                  distance_matrix: Dict, eta_matrix: Dict) -> Optional[Route]:
    ...
    def _calculate_simple_route_metrics(self, route: Route, distance_matrix: Dict, 
                                      eta_matrix: Dict) -> Tuple[float, int]:
    ...
    def route_splitting_operator(self, solution: Solution, distance_matrix: Dict, 
                               eta_matrix: Dict) -> Solution:
        ...
        for route in new_solution.routes:
            ...
            if (utilization > 90 or 
                not route.is_time_feasible(self.config.time_window_seconds) or
                len(route.parcels) > 8):  # Arbitrary threshold
                routes_to_split.append(route)

        # Try to split selected routes
        for route in routes_to_split:
            split_routes = self._try_split_route_for_cost(route, distance_matrix, eta_matrix)
            if split_routes and len(split_routes) > 1:
                ...
                if split_total_cost < original_total_cost * 1.1:  # Allow 10% cost increase
    ...
    def _try_split_route_for_cost(self, route: Route, distance_matrix: Dict, 
        ...
        for i, parcels_subset in enumerate([part1_parcels, part2_parcels]):
            ...
            best_route = self._create_best_route_for_parcels(
                parcels_subset, route.pickup_sequence, next_vehicle_id + i,
                distance_matrix, eta_matrix
            )
    ...
    def _create_best_route_for_parcels(self, parcels: List[Parcel], pickup_sequence: List[int],
                                     vehicle_id: int, distance_matrix: Dict, 
                                     eta_matrix: Dict) -> Optional[Route]:
        ...
        for vehicle_type, vehicle_spec in self.config.vehicle_specs.items():
            if vehicle_spec.capacity >= subset_size:
                ...
                if (route.is_time_feasible(self.config.time_window_seconds) and
                    route.cost_per_parcel < best_cost_per_parcel):
                    best_cost_per_parcel = route.cost_per_parcel
                    best_route = route

        return best_route

    def parcel_relocation_operator(self, solution: Solution, distance_matrix: Dict, 
                                 eta_matrix: Dict) -> Solution:
        ...
        for _ in range(min(5, len(new_solution.routes))):  # Limit attempts
            ...
            target_route = self.random.choice([r for r in new_solution.routes if r != source_route])
            ...
            if (target_route.total_size + parcel.size <= target_route.vehicle_spec.capacity and
                self._quick_time_check(target_route, parcel, distance_matrix, eta_matrix)):
                
                # Calculate cost impact
                original_cost = source_route.cost_per_parcel + target_route.cost_per_parcel
                
                # Estimate new costs
                new_source_cost = source_route.vehicle_spec.calculate_cost_per_parcel(
                    len(source_route.parcels) - 1
                ) if len(source_route.parcels) > 1 else 0
                ...
                if new_total_cost < original_cost:
                    source_route.remove_parcels([parcel])
                    target_route.add_parcels([parcel])
    ...
    def local_parcel_swap(self, solution: Solution, distance_matrix: Dict, 
                         eta_matrix: Dict) -> Solution:
        ...
        for _ in range(min(3, len(new_solution.routes))):  # Limit attempts
            ...
            if (route1.total_size - parcel1.size + parcel2.size <= route1.vehicle_spec.capacity and
                route2.total_size - parcel2.size + parcel1.size <= route2.vehicle_spec.capacity):
                
                # Calculate cost impact
                original_cost = route1.cost_per_parcel + route2.cost_per_parcel
                
                # Estimate new costs (same parcel counts)
                new_cost1 = route1.vehicle_spec.calculate_cost_per_parcel(len(route1.parcels))
                new_cost2 = route2.vehicle_spec.calculate_cost_per_parcel(len(route2.parcels))
                ...
                if self.random.random() < 0.3:  # 30% chance to try swap
                    route1.remove_parcels([parcel1])
                    route1.add_parcels([parcel2])
                    route2.remove_parcels([parcel2])
                    route2.add_parcels([parcel1])
                    break
