...
class UpdatedVehicleOptimizer:
    ...
    def optimize_vehicle_types_for_cost(self, solution: Solution) -> Solution:
        ...
        for route in optimized_solution.routes:
            ...
            if best_vehicle_type != route.vehicle_type:
                ...
                if self.verbose_logging:
                    print(f"  Route {route.vehicle_id}: {route.vehicle_type.value} -> {best_vehicle_type.value}")
                    print(f"    Cost per parcel: {old_cost_per_parcel:.2f} -> {route.cost_per_parcel:.2f}")
    ...
    def suggest_route_consolidation_opportunities(self, solution: Solution) -> List[Dict]:
        """
        ...
        cross_terminal_suggestions = self._analyze_cross_terminal_consolidation(solution)
    ...
    def _analyze_consolidation_for_terminal(self, terminal_id: int, 
                                          routes: List[Route]) -> Optional[Dict]:
            ...
            if vehicle_spec.capacity >= total_size:
                ...
                if consolidated_cost_per_parcel < best_cost_per_parcel:
                    ...
                    best_consolidation = {
                        ...
                        "improvement_percent": ((current_cost_per_parcel - consolidated_cost_per_parcel) / 
                                              current_cost_per_parcel) * 100,
                        "total_parcels": current_total_parcels,
                        "total_size": total_size,
                        "vehicle_capacity": vehicle_spec.capacity,
                        "utilization_percent": (total_size / vehicle_spec.capacity) * 100
                    }
    ...
    def _analyze_cross_terminal_consolidation(self, solution: Solution) -> List[Dict]:
        ...
        for i in range(len(routes)):
            for j in range(i + 1, len(routes)):
                ...
                if (route1.pickup_sequence and route2.pickup_sequence and
                    route1.pickup_sequence[0] == route2.pickup_sequence[0]):
                    continue
                    ...
                    if vehicle_spec.capacity >= combined_size:
                        ...
                        if improvement_percent > 5:  # At least 5% improvement
                            suggestions.append({
                                ...
                                "route1_terminal": route1.pickup_sequence[0] if route1.pickup_sequence else 0,
                                "route2_terminal": route2.pickup_sequence[0] if route2.pickup_sequence else 0,
                                "vehicle_type": vehicle_type.value,
                                "current_cost_per_parcel": current_cost_per_parcel,
                                "consolidated_cost_per_parcel": consolidated_cost_per_parcel,
                                ...
                                "vehicle_capacity": vehicle_spec.capacity,
                                "utilization_percent": (combined_size / vehicle_spec.capacity) * 100
                            })
    ...
    def calculate_fleet_efficiency_metrics(self, solution: Solution) -> Dict:
        ...
        for vtype, data in vehicle_type_analysis.items():
            if data["count"] > 0:
                data["average_cost_per_parcel"] = data["total_cost"] / data["total_parcels"]
                data["average_utilization"] = (data["total_used_capacity"] / data["total_capacity"]) * 100
                data["cost_efficiency_rank"] = data["average_cost_per_parcel"]  # Lower is better
        ...
        return {
            ...
            "consolidation_opportunities": self.suggest_route_consolidation_opportunities(solution)
        }
    ...
    def _generate_efficiency_recommendations(self, solution: Solution) -> List[Dict]:
        ...
        underutilized_routes = [r for r in solution.routes if r.get_utilization_percentage() < 50]
        if underutilized_routes:
            recommendations.append({
                ...
                "potential_savings": self._calculate_underutilization_savings(underutilized_routes)
            })
        ...
        for route in solution.routes:
            ...
            smaller_vehicles = [
                (vtype, spec) for vtype, spec in self.vehicle_specs.items()
                if spec.capacity >= min_capacity_needed and spec.capacity < current_capacity
            ]
            
            if smaller_vehicles:
                # Find the best smaller vehicle
                best_smaller = min(smaller_vehicles, 
                                 key=lambda x: x[1].calculate_cost_per_parcel(len(route.parcels)))
                oversized_routes.append((route, best_smaller))
        
        if oversized_routes:
            total_savings = sum(
                route.cost_per_parcel - smaller_spec.calculate_cost_per_parcel(len(route.parcels))
            ...
            recommendations.append({
                ...
                "affected_routes": [route.vehicle_id for route, _ in oversized_routes],
                "potential_savings_per_parcel": total_savings / len(oversized_routes)
            })
        
        # Check for consolidation opportunities
        consolidation_opportunities = self.suggest_route_consolidation_opportunities(solution)
        high_value_opportunities = [
            opp for opp in consolidation_opportunities 
        ...
        if high_value_opportunities:
            recommendations.append({
                ...
                "potential_improvement": sum(opp.get("improvement_percent", 0) for opp in high_value_opportunities)
            })
        
        # Check for balanced utilization
        utilizations = [r.get_utilization_percentage() for r in solution.routes]
        if utilizations:
            utilization_std = (sum((u - sum(utilizations)/len(utilizations))**2 for u in utilizations) / len(utilizations))**0.5
    ...
    def _calculate_underutilization_savings(self, underutilized_routes: List[Route]) -> float:
        ...
        avg_cost_per_route = sum(r.total_cost for r in underutilized_routes) / len(underutilized_routes)
    ...
    def optimize_vehicle_assignment_simple(self, solution: Solution) -> Solution:
        ...
        for route in optimized_solution.routes:
            ...
            if best_vehicle_type != route.vehicle_type:
                ...
                if self.verbose_logging:
                    print(f"Route {route.vehicle_id}: vehicle type optimized, "
            ...
            if len(routes) > 1:
                ...
                if consolidated_route:
                    ...
                    if consolidated_route.total_cost < original_total_cost:
                        ...
                        if self.verbose_logging:
                            print(f"Terminal {terminal_id}: consolidated {len(routes)} routes into 1, "
    ...
    def _try_simple_consolidation(self, routes: List[Route]) -> Optional[Route]:
        ...
        for vehicle_type, vehicle_spec in self.vehicle_specs.items():
            if vehicle_spec.capacity >= total_size:
                cost_per_parcel = vehicle_spec.calculate_cost_per_parcel(len(total_parcels))
                if cost_per_parcel < best_cost_per_parcel:
                    best_cost_per_parcel = cost_per_parcel
                    best_vehicle_type = vehicle_type
    ...
    def get_vehicle_type_recommendations(self, solution: Solution) -> Dict[VehicleType, Dict]:
        """Get recommendations for each vehicle type usage"""
        recommendations = {}
        
        for vehicle_type, vehicle_spec in self.vehicle_specs.items():
            # Find routes using this vehicle type
            routes_with_type = [r for r in solution.routes if r.vehicle_type == vehicle_type]
    ...
    def _find_optimal_parcel_range(self, vehicle_spec: VehicleSpec) -> Tuple[int, int]:
        ...
        optimal_range = [
            parcel_count for parcel_count, cost in costs
