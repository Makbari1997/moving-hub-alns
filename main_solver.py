...
from data_structures import (
    VehicleType, VehicleSpec, ProblemConfig, PickupTerminal, Parcel, Solution
)
...
class PerformanceOptimizedSolver:
    ...
    def solve_from_csv_optimized(
        ...
        max_iterations: int = 200,  # Reduced default
        performance_target_seconds: int = 60,  # Target completion time
    ) -> Dict:
        """
        ...
        if cost_per_hour_config is None:
            cost_per_hour_config = {
                "bike": 1200.0,      # Cost per hour
                "carbox": 2000.0,    # Cost per hour  
                "big-box": 1500.0    # Cost per hour
            }
        
        # Process data by polygon
        all_etas = {}
        all_distances = {}
        aggregated_results = {"polygon_results": {}}
        
        # Group by polygon if column exists
        ...
        for polygon_id, group in grouped_data:
            ...
            try:
                ...
                # Track matrices for visualization
                ...
                # Clean up large matrices from result
                if "distance_matrix" in result:
                ...
                if self.verbose_logging:
                    print(f"Polygon {polygon_id} completed in {polygon_time:.2f} seconds")
        ...
        if self.verbose_logging:
            ...
            for result in aggregated_results["polygon_results"].values():
                if "results_summary" in result:
                    total_vehicles += result["results_summary"].get("total_vehicles_used", 0)
                    total_cost += result["results_summary"].get("total_cost", 0.0)
            
            print(f"Total vehicles used: {total_vehicles}")
            print(f"Total cost: {total_cost:.2f}")
            print(f"Average cost per polygon: {total_cost / len(aggregated_results['polygon_results']):.2f}")
    ...
    def _solve_single_polygon(
        ...
        config, pickup_terminals, data_summary = self._create_problem_from_dataframe(
            df, max_knock, time_window_hours, cost_per_hour_config, 
            default_parcel_size, proximity_threshold_meters
        )
        
        if self.verbose_logging:
            print(f"  - Pickup terminals: {data_summary['num_pickup_terminals']}")
            print(f"  - Total parcels: {data_summary['total_parcels']}")
            print(f"  - Avg parcels per terminal: {data_summary['avg_parcels_per_terminal']:.1f}")
        
        # Step 2: Distance Matrix Building
        if self.verbose_logging:
            print("Step 2: Building distance matrix...")
        
        distance_matrix, eta_matrix = self._build_distance_matrices(pickup_terminals)
        
        # Step 3: Performance-Optimized Solving
        if self.verbose_logging:
            print("Step 3: Solving VRP...")
        
        # Adjust parameters based on instance size and performance target
        ...
        # Add matrices for visualization (will be removed later)
    ...
    def _create_problem_from_dataframe(
        ...
        # Define vehicle specifications with new cost model
        vehicle_specs = {}
        
        # Default capacities (can be made configurable)
        vehicle_capacities = {
            "bike": 28,
            "carbox": 212,
            "big-box": 44
    ...
    def _create_pickup_terminals(self, df: pd.DataFrame, processor: DataProcessor) -> List[PickupTerminal]:
    ...
    def _build_distance_matrices(self, pickup_terminals: List[PickupTerminal]) -> Tuple[Dict, Dict]:
    ...
    def benchmark_performance(
        self,
        df: pd.DataFrame,
        target_times: List[int] = [30, 60, 120],
        **kwargs
    ) -> Dict:
        """
        ...
        for target_time in target_times:
            ...
            result = self.solve_from_csv_optimized(
                df=df,
                performance_target_seconds=target_time,
                **kwargs
            )
            ...
            benchmark_run = {
                ...
                "total_vehicles": sum(
                    polygon_result.get("results_summary", {}).get("total_vehicles_used", 0)
                    for polygon_result in result["polygon_results"].values()
                ),
                "feasible_solutions": sum(
                    1 for polygon_result in result["polygon_results"].values()
                    if polygon_result.get("results_summary", {}).get("time_window_feasible", False)
                )
            }
        ...
        # Generate summary
        successful_runs = [run for run in benchmark_results["benchmark_runs"] if run["target_met"]]
        
        benchmark_results["summary"] = {
            "total_runs": len(benchmark_results["benchmark_runs"]),
            "successful_runs": len(successful_runs),
            "success_rate": len(successful_runs) / len(benchmark_results["benchmark_runs"]) * 100,
            "fastest_time": min(run["actual_time_seconds"] for run in benchmark_results["benchmark_runs"]),
            "best_cost": min(run["total_cost"] for run in benchmark_results["benchmark_runs"]),
            "recommended_target": min(target_times) if successful_runs else max(target_times)
        }
