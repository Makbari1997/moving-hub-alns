...
class GlobalRouteOptimizer:
        ...
        try:
            ...
            if enable_route_merging and len(sequence_optimized.routes) > 1:
                merged_solution = self._attempt_route_merging(
                    sequence_optimized, distance_matrix, eta_matrix
                )

                # Choose best solution based on cost per parcel
                if (
                    merged_solution.average_cost_per_parcel
                    < sequence_optimized.average_cost_per_parcel
                ):
    ...
    def _optimize_single_route_sequence(
        ...
        try:
            ...
            if solution:
                return self._extract_optimized_route(
        ...
        for terminal_id, routes in terminal_groups.items():
            if len(routes) > 1:
                merged_routes = self._merge_routes_for_terminal(
                    routes, distance_matrix, eta_matrix
                )
    ...
    def _merge_routes_for_terminal(
        self, routes: List[Route], distance_matrix: Dict, eta_matrix: Dict
    ) -> Optional[List[Route]]:
        ...
        for vehicle_type, vehicle_spec in self.config.vehicle_specs.items():
            if vehicle_spec.capacity >= total_size:
                ...
                if merged_route:
                    ...
                    # Check feasibility
                    if optimized_merged.is_time_feasible(
                ...
                for vehicle_type, vehicle_spec in self.config.vehicle_specs.items():
                    if vehicle_spec.capacity >= combined_size:
                        ...
                        if merged_route:
                            ...
                            if (
                                optimized_merged.is_time_feasible(
                                    self.config.time_window_seconds
                                )
                                and optimized_merged.cost_per_parcel
                                < (route1.cost_per_parcel + route2.cost_per_parcel) / 2
                            ):
                                ...
                                if (
                                    test_solution.average_cost_per_parcel
            ...
            if len(routes) > 1:
                ...
                for vehicle_type, vehicle_spec in self.config.vehicle_specs.items():
                    if vehicle_spec.capacity >= total_size:
                        ...
                        if merged_route.cost_per_parcel < sum(
                            r.cost_per_parcel for r in routes
                        ) / len(routes):
                            # Apply merging
                            for route in routes:
...
class GlobalOptimizationController:
    """
    Controller for managing when and how to apply global optimization
    Enhanced with vehicle type optimization
    """

    def __init__(self, config: ProblemConfig):
        self.config = config
        self.optimizer = GlobalRouteOptimizer(config)
        self.vehicle_optimizer = UpdatedVehicleOptimizer(config)  # ADD THIS
        self.last_optimization_iteration = -1
        self.optimization_interval = 30  # Optimize every 30 iterations
        self.min_improvement_threshold = 0.01  # 1% improvement threshold
    ...
    def should_optimize(
        ...
        # Optimize when stagnating
        # if iterations_since_improvement >= 20:
        #     print("Should optimize due to stagnation")
        #     return True
    ...
    def optimize_if_needed(
        ...
        """
        Apply global optimization if conditions are met
        Now includes both route optimization and vehicle type optimization
        ...
        # Step 1: Apply global route optimization (sequences + merging)
        ...
        route_improvement = (original_cost - route_optimized_solution.average_cost_per_parcel) / original_cost
        if route_improvement >= self.min_improvement_threshold:
            best_solution = route_optimized_solution
            print(f"Route optimization improved cost per parcel by {route_improvement*100:.1f}%")

        # Step 2: Apply vehicle type optimization (if enabled and conditions are met)
        if self.check_enable_vehicle_optimization() and self._should_apply_vehicle_optimization(iteration):
            print("Vehicle type optimization in progress...")
            
            vehicle_optimized_solution = self.vehicle_optimizer.optimize_vehicle_types_for_cost(best_solution)
            
            # Check if vehicle optimization improved
            vehicle_improvement = (best_solution.average_cost_per_parcel - vehicle_optimized_solution.average_cost_per_parcel) / best_solution.average_cost_per_parcel
            if vehicle_improvement >= self.min_improvement_threshold:
                best_solution = vehicle_optimized_solution
                print(f"Vehicle optimization improved cost per parcel by {vehicle_improvement*100:.1f}%")
            else:
        ...
        total_improvement = (original_cost - best_solution.average_cost_per_parcel) / original_cost
    ...
    def final_optimization(
        ...
        """
        Apply final comprehensive optimization before returning solution
        Includes both route and vehicle optimization
        """
        ...
        # Step 1: Final route optimization
        print("Final route optimization...")
        route_optimized = self.optimizer.optimize_solution_globally(
            solution, distance_matrix, eta_matrix, enable_route_merging=True
        )
        
        route_improvement = (original_cost - route_optimized.average_cost_per_parcel) / original_cost
        if route_improvement >= self.min_improvement_threshold:
            best_solution = route_optimized
            print(f"Final route optimization improved by {route_improvement*100:.1f}%")

        # Step 2: Final vehicle optimization
        if self.enable_vehicle_optimization:
            print("Final vehicle optimization...")
            vehicle_optimized = self.vehicle_optimizer.optimize_vehicle_types_for_cost(best_solution)
            
            vehicle_improvement = (best_solution.average_cost_per_parcel - vehicle_optimized.average_cost_per_parcel) / best_solution.average_cost_per_parcel
            if vehicle_improvement >= self.min_improvement_threshold:
                best_solution = vehicle_optimized
                print(f"Final vehicle optimization improved by {vehicle_improvement*100:.1f}%")
    ...
    def configure_performance_mode(self, fast_mode: bool = True):
        """
        Configure optimization for performance vs quality trade-off
        ...
        if fast_mode:
            ...
            self.vehicle_optimization_frequency = 3  # Less frequent vehicle optimization
            self.enable_vehicle_optimization = True  # Keep it enabled but less frequent
            print("Performance mode: FAST - Less frequent but faster optimization")
        else:
            ...
            self.enable_vehicle_optimization = True
            print("Performance mode: QUALITY - More frequent and thorough optimization")
    ...
    def check_enable_vehicle_optimization(self):
        ...
        self.enable_vehicle_optimization = True
        print("Vehicle optimization enabled for better solution quality")
