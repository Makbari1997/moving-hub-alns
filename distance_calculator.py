...
class DistanceCalculator:
    ...
    def build_matrices_smapp(
        ...
        for source_loc in locations:
            ...
            for i in range(0, len(target_locations), batch_size):
                ...
                payload = {
                    "sources": [{"lat": source_loc[0], "lon": source_loc[1]}],
                    "targets": [
                        {"lat": target[0], "lon": target[1]} for target in batch_targets
                    ],
                }
                    ...
                    if response is not None:
                        ...
                        for j, target_result in enumerate(sources_to_targets):
                            ...
                            if target_result["status"] == "Success":
                                ...
                                distance_matrix[source_loc][target_loc] = target_result[
                                    "distance"
                                ]
                            else:
                                # Handle failed calculations - you might want to set to None or a default value
                                print(
                                    f"Failed to calculate distance from {source_loc} to {target_loc}"
                                )
                                eta_matrix[source_loc][target_loc] = None
                                distance_matrix[source_loc][target_loc] = None
                    else:
                        print(
                            f"All retry attempts failed for batch starting at index {i}"
                        )
    ...
    def build_matrices(
        ...
        if constants.USE_HAVERSINE:
            distance_matrix = DistanceCalculator.build_distance_matrix_haversine(
                locations
            )
            eta_matrix = DistanceCalculator.build_eta_matrix(locations)
        else:
            distance_matrix, eta_matrix = DistanceCalculator.build_matrices_smapp(
                locations
            )
        return distance_matrix, eta_matrix
